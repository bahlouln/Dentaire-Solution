...
function PasswordField({
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {errors.motDePasse && (
        <p id="motDePasse-error" className="text-xs text-rose-600">
          {errors.motDePasse}
        </p>
      )}
    </div>
...
export default function AddDentistes() {
  ...
  const validators = {
    ...
    numero: (v) =>
      /^[0-9]{8}$/.test(v || "") ? "" : "Le numéro doit contenir 8 chiffres.",
    motDePasse: (v) => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      setServerError(
        err?.response?.data?.message || "Erreur lors de l'ajout du dentiste"
      );
    } finally {
  ...
  if (!user)
    return (
      <p className="text-center mt-4 text-red-600">
        Vous devez être connecté pour ajouter un dentiste.
      </p>
    );
        ...
        <div className="flex items-center justify-between gap-3 rounded-t-2xl border-b border-slate-200 bg-gradient-to-r from-indigo-50 to-sky-50 px-5 py-4">
          <div>
            ...
            <p className="text-sm text-slate-600">
              Renseignez les informations du nouveau dentiste.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={onSubmit} noValidate className="px-5 py-6">
          ...
          <div className="mt-6 flex items-center justify-end gap-3">
            <button
              type="button"
              onClick={() => navigate(-1)}
              className="inline-flex items-center justify-center rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-4 focus:ring-slate-100"
            >
              Annuler
            </button>
            <button
              ...
              className="inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm font-semibold shadow-sm bg-indigo-600 text-white hover:bg-indigo-500 focus:outline-none focus:ring-4 focus:ring-indigo-200 disabled:cursor-not-allowed disabled:opacity-60"
            >
