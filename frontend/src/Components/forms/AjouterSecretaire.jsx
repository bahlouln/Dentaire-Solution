...
/**
 * AjouterSecretaire (Tailwind, JS, ergonomique & accessible)
 * - Validation client (nom, email, mot de passe fort)
 * - Erreurs contextuelles au blur + contrôle global au submit
 * - Bouton avec état de chargement, messages succès/erreur
 * - Toggle visibilité mot de passe
 * - Layout responsive (1 -> 2 colonnes)
 */
export default function AjouterSecretaire() {
    ...
    const Field = ({ id, label, type = "text", required = false, placeholder = "", autoComplete }) => (
    ...
    const PasswordField = () => (
        <div className="flex flex-col gap-1">
            ...
            <p id="pwd-hint" className="text-xs text-slate-500">Doit inclure minuscule, majuscule et chiffre.</p>
            {errors.motDePasse && (
            ...
            <div className="rounded-2xl border border-slate-200 bg-white/70 backdrop-blur shadow-sm">
                ...
                <form onSubmit={onSubmit} noValidate className="px-5 py-6">
                    <div className="grid grid-cols-1 gap-4 sm:gap-5 md:grid-cols-2">
                        <Field id="nom" label="Nom" required placeholder="Ex. Ben Salah" autoComplete="name" />
                        <Field id="email" label="Email" type="email" required placeholder="exemple@mail.com" autoComplete="email" />
                        <PasswordField />
                        <Field id="bureau" label="Bureau (facultatif)" placeholder="Ex. B-203" autoComplete="off" />
                    </div>
