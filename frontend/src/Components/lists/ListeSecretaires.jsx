...
export default function ListeSecretaires() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const res = await axios.get(`${API_URL}/secretaires/dentiste`, {
                ...
                if (!cancelled && !axios.isCancel(e)) {
                    let errorMsg = "Impossible de récupérer les secrétaires.";
                    if (e.response?.status === 401) {
                        errorMsg = "Session expirée. Veuillez vous reconnecter.";
                        // Optionnel : logout automatique
                        localStorage.removeItem("token");
                        // Redirection vers login si besoin : window.location.href = "/login";
    ...
    const saveRow = async (id) => {
        ...
        try {
            const token = localStorage.getItem("token");
            await axios.put(`http://localhost:5000/secretaires/${id}`, formData, {
                    ...
                    {secretaires.length === 0 ? (
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full border-separate border-spacing-y-2">
                                <caption className="sr-only">Secrétaires du dentiste connecté</caption>
                                <thead>
                                <tr>
